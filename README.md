...
Nginx = 5008 rps
```
С учетом географического расположения, а также деления баз, лучше будет расположить сервера в двух датацентрах - один в Москве и МО, ориентированный как на столицу, так и на СПб и ЛО, Казань, Нижний Новгород. Другой - ближе к Дальнему Востоку, в Новосибирске или Красноярске [11].
Для каждого сервера будем брать один сервер и один резерв на один ДЦ, если не указано иного.
#### Nginx
Nginx позволяет поддерживать 9871 rps HTTPS запросов размером с 100 Кб на двух ядрах и 19355 rps на 4х [12]. Предполагая возможность обесточивания одного из ДЦ, итоговая конфигурация сервера:
| **Параметр** | **Значение** |
| :-: | :-: |
| CPU (cores) | 4 |
| Network GbE | 40 |
...
#### Авторизация
На данном сервере необходимо большое количество оперативной памяти для хранения сессий - минимум 32 Гб.
...
#### Объявления
Сервис объявлений является самым нагруженным, также здесь необходимо использовать 64-х битные сервера для хранения базы. Для неё же, с учетом шардинга, можно поставить 1 Тб SSD диски. MongoDB также требует много оперативной памяти для кэширования. Redis для просмотров также нужно большое количество оперативной памяти. Есть смысл запустить базы данных на отдельных машинах. Тогда
...
#### Пользователи
Также один из наиболее загруженных сервисов. К базе, связанной с ними, понадобится машина, способная поддерживать большое число соединений - к ней обращаются несколько сервисов.
...
### Заключение
В ходе работы было выполнено проектирование высоконагруженной системы - интернет-сервиса для размещения объявлений Авито.
...
11. https://www.datacentermap.com/
12. https://www.nginx.com/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers/
