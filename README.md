...
Итого: 221.35 Гб
```
С учетом максимального размера базы данных, в ближайшее время дополнительное разделение из-за первышения объема не потребуется.
 
Таким образом, базу данных можно шардировать по регионам - балансировщик нагрузки при обращении дополнительно будет получать ключ, по которому будет отправлять запрос на соответствующий сервер. Для 
...
Остальные данные (чаты, отзывы и прочее) хранятся рядом с пользователями в реляционных базах - имеют постоянную схему данных.
...
* **Сессии** будут храниться в key-value хранилище, **Redis**. В качестве ключа будет email пользователя, в качестве значения - значения id пользователя, его куки и времени начала сессии.
...
7. https://ru.wikipedia.org/wiki/Go
8. https://trends.google.com/trends/explore?cat=5&date=today%205-y&q=React,Angular,Vue,Backbone,Ember
9. http://backendconf.ru/moscow-rit/2019/abstracts/5319
